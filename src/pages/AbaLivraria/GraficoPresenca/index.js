...
export default function GraficoPresenca({ match }) {
  ...
  const contadorPresenca = async (listPresenca, alunos) => {
    ...
    alunos.map((aluno) => {
      ...
      porcentagemTotal = (contador * 100) / 13

    });
    setHidden(false);
    setPresenca(novaLista);
    console.log(novaLista);
    montaGrafico(alunos, porcentagemTotal)
    setIsLoading(false);
  };
  const montaGrafico = (alunos, porcentagemTotal) => {
    console.log(alunos.length, porcentagemTotal)
  }
